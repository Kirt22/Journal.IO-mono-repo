import { Request, Response } from "express";
import {
  apiResponse,
  API_MESSAGES,
} from "../../helpers/commonHelper.helpers";
import { getHomeOfferConfig } from "./admin.service";

const getHomeOfferConfigController = async (
  req: Request & { user?: { _id?: string } },
  res: Response
) => {
  try {
    const userId = req.user?._id?.toString();

    if (!userId) {
      return res.status(401).json(apiResponse(false, API_MESSAGES.unauthorized, {}));
    }

    const config = await getHomeOfferConfig();

    return res
      .status(200)
      .json(apiResponse(true, "Home offer settings loaded.", config));
  } catch (error) {
    console.error("Error in getHomeOfferConfigController:", error);
    return res
      .status(500)
      .json(apiResponse(false, API_MESSAGES.internalError, {}));
  }
};

export { getHomeOfferConfigController };
