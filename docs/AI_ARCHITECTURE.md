...
- **Ask Jade (premium)** is the conversational surface of the same reflection companion, reached by tapping the Home hero orb. Backend `services/ask-jade/`, mounted at `/api/v1/ask-jade`; collections `jade_sessions` and `jade_messages` (row-per-message, so a long chat never rewrites a growing document). Frontend `screens/jade/AskJadeScreen.tsx` + `store/slices/askJadeSlice.ts`.
  ...
  - **Rich replies are server-owned data.** Jade messages carry encrypted `blocks` for styled points, overview stats, 7/30-day mood trends, 30-day/all-time mood distribution, and 7-day writing activity. The server detects explicit visualization intent and builds values from insights/mood services; the model supplies prose and optional point copy but cannot supply chart numbers. `text` remains a flattened legacy fallback.
